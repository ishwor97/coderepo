...
public class SuperMarketImpl implements Supermarket {
	...
	private HashMap<Product, Integer> productListCounter = new HashMap<Product, Integer>();
	...
	public int checkOut(String items) {
		...
		for (char c : charArray) {
			...
			if (!isAdded) {
				...
				int count = productListCounter.get(p);
				productListCounter.put(p, ++count);

			} else {
				// Adding product to Map for the first time.
				productListCounter.put(getProduct(c), 1);
			}
		}
		/* End of Logic A: */

		/* Logic B: Logic for calculating Price */
		for (Entry<Product, Integer> entry : productListCounter.entrySet()) {
